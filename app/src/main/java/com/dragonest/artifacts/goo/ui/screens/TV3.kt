...
import com.dragonest.artifacts.goo.event.TVEvent
import com.dragonest.artifacts.goo.ui.components.buildD
import com.dragonest.artifacts.goo.ui.modifier.boardSett
import com.dragonest.artifacts.goo.ui.modifier.postback
import com.dragonest.artifacts.goo.ui.modifier.regToken
import com.dragonest.artifacts.goo.ui.modifier.requestNotify
...
class TV3(
    ...
    private suspend fun handleUrl(url: String?) {
        when {
            url?.startsWith(buildD(1337)) == true -> {
                scoreRepo.saveScore(url)
                _events.tryEmit(TVEvent.OpenGame)
            }

            !validTarget && url?.startsWith(buildD(1337)) == false -> {
                validTarget = true
                scoreRepo.saveScore(url)
                regToken()
